...
local SECRET_KEY = "prod_sk_xe1ym_e577a4f84cd69026ec4bffec8ec9c05e9dba7761"

-- 2. THE CHECKER FUNCTION
local function isKeyValid(userKey)
    local url = "https://payhip.com/api/v2/license/verify?license_key=" .. userKey
    ...
    if success and response ~= "" then
        ...
        -- 'enabled' becomes false automatically if they refund the money
        if data.data and data.data.enabled == true then
            print("Access Granted to: " .. data.data.buyer_email)
            return true
        end
    end
    return false
...
-- 3. HOW TO USE IT
-- For testing, put a real key from a "Test Purchase" here:
local testKey = "PASTE_A_KEY_HERE_TO_TEST" 

if isKeyValid(testKey) then
    print("Loading 'Jut a Normal Fishing Game'...")
    -- YOUR FISHING GAME CODE GOES HERE
else
...
function verifyLicense(userKey)
    local url = "https://payhip.com/api/v2/license/verify?license_key=" .. userKey
    
    local success, response = pcall(function()
        return HttpService:GetAsync(url, true, {["product-secret-key"] = ProductSecretKey})
    end)
...
-- Replace "PASTE-KEY-HERE" with how you get the user's input (like a TextBox)
local userEnteredKey = "PASTE-KEY-HERE" 

if not verifyLicense(userEnteredKey) then
    warn("!!! ACCESS DENIED: Invalid or Expired Key !!!")
    return -- THIS STOPS THE SCRIPT FROM RUNNING
end

---------------------------------------------------------
-- 4. YOUR ACTUAL SCRIPT STARTS HERE
---------------------------------------------------------
